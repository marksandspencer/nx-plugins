...
export default async function runExecutor(
  ...
  const runner = options.packageRunner ?? executorSchema.properties.packageRunner.default;
  const command = [runner, 'playwright', 'test', path, `--config ${options.e2eFolder}/${config}`]
    .concat(args)
    .join(' ');
